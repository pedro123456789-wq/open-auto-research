...
function StatusBadge({ status }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `badge badge-${status}`, children: LABELS[status] ?? status });
}
function Sidebar({ runs, selectedDir, onSelect, anyRunning, onStartClick }) {
  const groups = runs.reduce((acc, r2) => {
    (acc[r2.run_name] ??= []).push(r2);
    return acc;
  }, {});
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: "sidebar", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "sidebar-title", children: "Runs" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "sidebar-runs", children: [
      Object.keys(groups).length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { padding: "12px 14px", color: "var(--text-muted)", fontSize: 12 }, children: "No runs yet" }),
      Object.entries(groups).map(([name, groupRuns]) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "sidebar-group-label", children: name }),
        groupRuns.map((run) => {
          ...
          return /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              ...
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "sidebar-item-label", children: /* @__PURE__ */ jsxRuntimeExports.jsx(StatusBadge, { status: run.status }) }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "sidebar-item-meta", children: [
                  ts,
                  " · ",
                  run.node_count,
                  " nodes · best ",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "sidebar-footer", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
        ...
        children: anyRunning ? "● Running…" : "+ New Run"
      }
    ) })
  ] });
}
...
function XYResizer({ domNode, nodeId, getStoreItems, onChange, onEnd }) {
  ...
  function update({ controlPosition, boundaries, keepAspectRatio, resizeDirection, onResizeStart, onResize, onResizeEnd, shouldResize }) {
    ...
    const dragHandler = drag().on("start", (event) => {
      ...
      const { width, height, x: x2, y: y2 } = getDimensionsAfterResize(startValues, params.controlDirection, pointerPosition, params.boundaries, params.keepAspectRatio, nodeOrigin, nodeExtent, childExtent);
...
function Pane({ isSelecting, selectionKeyPressed, selectionMode = SelectionMode.Full, panOnDrag, autoPanOnSelection, paneClickDistance, selectionOnDrag, onSelectionStart, onSelectionEnd, onPaneClick, onPaneContextMenu, onPaneScroll, onPaneMouseEnter, onPaneMouseMove, onPaneMouseLeave, children: children2 }) {
  ...
  const onPointerDownCapture = (event) => {
    ...
    const userSelectionStartPosition = pointToRendererPoint({ x: x2, y: y2 }, transform2);
    store.setState({
      userSelectionRect: {
        width: 0,
        height: 0,
  ...
  function autoPan() {
    ...
    const [x2, y2] = calcAutoPan(position.current, containerBounds.current, autoPanSpeed);
    panBy2({ x: x2, y: y2 }).then((panned) => {
...
function layoutNodes(rows) {
  ...
  for (const n2 of rows) {
    const r2 = n2.round ?? 0;
    (byRound.get(r2) ?? byRound.set(r2, []).get(r2)).push(n2);
  }
  ...
  for (const round of rounds) {
    ...
    group.forEach((n2, i) => {
      positions.push({ id: n2.node_id, x: startX + i * (NODE_W + H_GAP), y: round * (NODE_H + V_GAP) });
    });
  }
  return positions;
}
function NodeTree({ nodes, selectedNodeId, onSelect }) {
  const { flowNodes, flowEdges } = reactExports.useMemo(() => {
    const positions = layoutNodes(nodes);
    const posMap = new Map(positions.map((p2) => [p2.id, p2]));
    const flowNodes2 = nodes.map((n2) => {
      const pos = posMap.get(n2.node_id) ?? { x: 0, y: 0 };
      const color2 = accuracyColor(n2.accuracy);
      const isSelected = n2.node_id === selectedNodeId;
      return {
        id: n2.node_id,
        position: { x: pos.x, y: pos.y },
        data: { label: `${n2.node_id}
${(n2.accuracy * 100).toFixed(1)}%` },
        style: {
          background: isSelected ? "#7c6af7" : "#1a1a22",
          border: `2px solid ${isSelected ? "#a89cf7" : color2}`,
          borderRadius: 8,
          color: "#e4e4ef",
          fontSize: 11,
          width: NODE_W,
    ...
    const flowEdges2 = nodes.filter((n2) => n2.parent_id).map((n2) => ({
      id: `${n2.parent_id}->${n2.node_id}`,
      source: n2.parent_id,
      target: n2.node_id,
      style: { stroke: "#2e2e3e", strokeWidth: 1.5 },
      animated: n2.node_id === selectedNodeId
    }));
    return { flowNodes: flowNodes2, flowEdges: flowEdges2 };
  }, [nodes, selectedNodeId]);
  if (nodes.length === 0) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty", style: { height: "100%" }, children: "No nodes yet" });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tree-panel", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
    index,
    {
      nodes: flowNodes,
      edges: flowEdges,
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Background, { color: "#2e2e3e", gap: 24 }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(Controls, { style: { background: "#1a1a22", border: "1px solid #2e2e3e" } }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          MiniMap,
          {
            nodeColor: (n2) => n2.style?.border?.split(" ")[2] ?? "#444",
...
function NodeDetail({ node }) {
  if (!node) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-panel", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty", style: { height: "100%" }, children: "Select a node" }) });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-panel", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-section", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-accuracy", children: [
        (node.accuracy * 100).toFixed(1),
        "%"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { color: "var(--text-muted)", fontSize: 12, marginTop: 2 }, children: [
        node.correct,
        "/",
        node.total,
        " correct"
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-section", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-label", children: "Node" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-value", style: { fontFamily: "monospace" }, children: node.node_id })
    ] }),
    node.parent_id && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-section", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-label", children: "Parent" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-value", style: { fontFamily: "monospace" }, children: node.parent_id })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-section", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-label", children: "Round" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-value", children: node.round === 0 ? "Root (seed)" : `Round ${node.round}` })
    ] }),
    node.model && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-section", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-label", children: "Model" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-value", children: node.model })
    ] }),
    node.reasoning && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-section", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-label", children: "Reasoning" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-value", style: { whiteSpace: "pre-wrap", fontSize: 12, color: "var(--text-muted)" }, children: [
        node.reasoning.slice(0, 600),
        node.reasoning.length > 600 ? "…" : ""
      ] })
    ] }),
    node.novelty && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-section", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-label", children: "Novelty" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-value", style: { whiteSpace: "pre-wrap", fontSize: 12, color: "var(--text-muted)" }, children: [
        node.novelty.slice(0, 400),
        node.novelty.length > 400 ? "…" : ""
      ] })
    ] })
  ] });
}
function RunView({ run, nodes }) {
  const [selectedNodeId, setSelectedNodeId] = reactExports.useState(null);
  const selectedNode = nodes.find((n2) => n2.node_id === selectedNodeId) ?? null;
  const bestAcc = run.best_accuracy > 0 ? `${(run.best_accuracy * 100).toFixed(1)}%` : "—";
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "run-view", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "run-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "run-title", children: run.run_name }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { marginTop: 4 }, children: /* @__PURE__ */ jsxRuntimeExports.jsx(StatusBadge, { status: run.status }) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "run-meta", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "run-meta-item", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "run-meta-label", children: "Best accuracy" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "run-meta-value", style: { color: "var(--accent)" }, children: bestAcc })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "run-meta-item", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "run-meta-label", children: "Nodes" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "run-meta-value", children: run.node_count })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "run-meta-item", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "run-meta-label", children: "Started" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "run-meta-value", style: { fontSize: 12 }, children: run.started_at ? new Date(run.started_at).toLocaleString() : "—" })
        ] })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "run-body", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        NodeTree,
        {
          nodes,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(NodeDetail, { node: selectedNode })
    ] })
  ] });
}
function StartRunDialog({ onClose, onStarted }) {
  const [runName, setRunName] = reactExports.useState("");
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dialog-overlay", onClick: (e) => e.target === e.currentTarget && onClose(), children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dialog", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "Start New Run" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("label", { children: "Run name" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "input",
      {
        ref: inputRef,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { marginTop: 6, fontSize: 11, color: "var(--text-muted)" }, children: "Must match folders in backend/baselines/, backend/evaluators/, backend/improvement_agents/" }),
    error && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dialog-error", children: error }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dialog-actions", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn", onClick: onClose, disabled: loading, children: "Cancel" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn btn-primary", onClick: handleStart, disabled: loading || !runName.trim(), children: loading ? "Starting…" : "Start" })
    ] })
  ] }) });
}
function App() {
  const [runs, setRuns] = reactExports.useState([]);
  const [selectedDir, setSelectedDir] = reactExports.useState(null);
  const [nodes, setNodes] = reactExports.useState([]);
  const [anyRunning, setAnyRunning] = reactExports.useState(false);
  const [showDialog, setShowDialog] = reactExports.useState(false);
  const refreshRuns = reactExports.useCallback(async () => {
    const all = await window.api.listRuns();
    setRuns(all);
    setAnyRunning(all.some((r2) => r2.status === "running"));
  }, []);
  const refreshNodes = reactExports.useCallback(async (runDir) => {
    const n2 = await window.api.getNodes(runDir);
    setNodes(n2);
  }, []);
  reactExports.useEffect(() => {
    refreshRuns();
    ...
    window.api.onRunUpdated((updated) => {
      setRuns((prev) => prev.map((r2) => r2.run_dir === updated.run_dir ? updated : r2));
      setAnyRunning((prev) => {
  ...
  const selectedRun = runs.find((r2) => r2.run_dir === selectedDir) ?? null;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "layout", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("main", { className: "main-area", children: selectedRun ? /* @__PURE__ */ jsxRuntimeExports.jsx(RunView, { run: selectedRun, nodes }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty", children: "Select a run from the sidebar" }) }),
    showDialog && /* @__PURE__ */ jsxRuntimeExports.jsx(
      StartRunDialog,
      {
